...
# Fonction pour extraire les URLs des produits d'une page de catégorie
...
url_categorie_principale = 'http://books.toscrape.com/catalogue/category/books_1/index.html'

# Créez un fichier CSV unique avec les en-têtes en dehors de la boucle
with open('all_categorie_data.csv', mode='w', newline='', encoding='utf-8') as csv_file:
    ...
    response = requests.get(url_categorie_principale)
    soup = BeautifulSoup(response.text, 'html.parser')

    side_categories = soup.find(class_='side_categories')

    if side_categories:
        categorie_links = side_categories.find_all('a')
        for link in categorie_links:
            categorie_titre = link.get_text().strip()
            categorie_url_complet = 'http://books.toscrape.com/catalogue' + link['href']
            print(f"{categorie_titre}: {categorie_url_complet}")

            urls_produits = extraire_urls_produits(categorie_url_complet)
            ...
            for product_url in urls_produits:
                ...
                if upc_element:
                    upc = upc_element.find_next("td").text.strip()
                else:
                    upc = "Non disponible"
